  ...
  return (
    <header className="flex max-w-3xl items-start gap-3">
      ...
      <div className="min-w-0">
        <p className="text-sm font-bold uppercase text-muted-foreground">{eyebrow}</p>
        <h3 className="mt-1 text-2xl font-bold leading-tight text-foreground sm:text-3xl">{title}</h3>
...
function FormulaText({ children }: { children: ReactNode }) {
  return <p className="mt-1 text-base leading-relaxed text-muted-foreground">{children}</p>;
}
  ...
  return (
    <div className="relative isolate flex min-h-[34rem] w-full items-center overflow-hidden rounded-lg lg:h-full lg:min-h-0">
      ...
      <div className="w-[82%] max-w-md px-5 py-10 sm:w-[52%] sm:px-10">
        ...
        <p className="mt-4 max-w-sm text-lg font-semibold leading-relaxed text-slate-900 dark:text-slate-100">
          {content.cover.description}
        </p>
        <div className="mt-7 flex items-center gap-2 text-base font-bold text-slate-950 dark:text-white">
      ...
      <div className="grid items-center gap-7 md:grid-cols-[0.8fr_1.2fr]">
        ...
        <div className="space-y-3">
          {content.whyQuestions.strengths.map((text, itemIndex) => (
            ...
            >
              <span className="text-base font-black text-foreground">0{itemIndex + 1}</span>
      ...
      <div className="relative space-y-4 md:pl-16">
        ...
        {content.classificationAxes.map((axis, axisIndex) => (
          ...
          >
            ...
            <div>
              <p className="text-sm font-bold text-muted-foreground">{content.twoAxes.axisLabel} {axisIndex + 1}</p>
              <h4 className="mt-1 text-xl font-bold text-foreground">{axis.title}</h4>
            </div>
            <div>
              <p className="text-base text-muted-foreground">{axis.description}</p>
    ...
    <div className="flex h-full w-full flex-col justify-center gap-6 p-5 sm:p-8">
      ...
      <div className="grid gap-4 md:grid-cols-[1fr_auto_1fr] md:items-stretch">
        <section className="rounded-lg border border-sky-200 bg-sky-50/70 p-5 dark:border-sky-900 dark:bg-sky-950/30">
          ...
          <p className="mt-3 text-base leading-relaxed text-muted-foreground">{content.answerRangeGuide.closed.definition}</p>
          <p className="mt-5 border-t border-sky-200 pt-4 text-base font-semibold leading-relaxed text-foreground dark:border-sky-800">
            {content.answerRangeGuide.closed.example}
          </p>
        </section>

        <div className="flex items-center justify-center text-sm font-black text-muted-foreground" aria-hidden="true">
        ...
        <section className="rounded-lg border border-emerald-200 bg-emerald-50/70 p-5 dark:border-emerald-900 dark:bg-emerald-950/30">
          ...
          <p className="mt-3 text-base leading-relaxed text-muted-foreground">{content.answerRangeGuide.open.definition}</p>
          <p className="mt-5 border-t border-emerald-200 pt-4 text-base font-semibold leading-relaxed text-foreground dark:border-emerald-800">
            ...
            <li key={type} className="contents">
              <div className={cn("flex min-h-32 flex-col justify-between rounded-lg border p-4", style.soft, style.border)}>
                ...
                <div>
                  ...
                  <p className="mt-1 text-base text-muted-foreground">{content.inquiryDepth.descriptions[type]}</p>
                </div>
              </div>
  ...
  return (
    <div className="grid h-full w-full gap-7 p-5 sm:p-8 md:grid-cols-[15rem_1fr] md:items-center">
      <div className="flex min-h-52 flex-col justify-between rounded-lg bg-sky-600 p-6 text-white">
        ...
        <div>
          <p className="text-base font-semibold text-sky-100">{content.factualDefinition.action}</p>
          <h3 className="mt-1 text-3xl font-black">{label}</h3>
        </div>
      </div>

      <div>
        <p className="text-sm font-bold text-sky-700 dark:text-sky-300">{content.factualDefinition.eyebrow}</p>
        ...
        <div className="mt-5 rounded-lg border-l-4 border-sky-500 bg-sky-50 px-4 py-3 text-base font-semibold leading-relaxed text-sky-950 dark:bg-sky-950/40 dark:text-sky-100">
      ...
      <ol className="overflow-hidden rounded-lg border border-sky-200 bg-background dark:border-sky-900">
        {guide.formulas.map((formula, formulaIndex) => (
          ...
          >
            ...
            <div className="min-w-0">
              <h4 className="text-base font-bold text-foreground">{formula.title}</h4>
              <FormulaText>
...
function ConceptualDefinitionSlide({ content, label }: { content: QuestionDetectiveContent; label: string }) {
  ...
  return (
    <div className="grid h-full w-full gap-8 p-5 sm:p-8 md:grid-cols-[1fr_1.15fr] md:items-center">
      ...
      <div>
        <p className="text-sm font-bold text-emerald-700 dark:text-emerald-300">{content.conceptualDefinition.eyebrow}</p>
      ...
      <ol className="relative space-y-3 pl-12 before:absolute before:bottom-5 before:left-[1.12rem] before:top-5 before:w-px before:bg-emerald-300 dark:before:bg-emerald-800">
        {guide.formulas.map((formula, formulaIndex) => (
          <li key={formula.title} className="relative rounded-lg border border-emerald-200 bg-emerald-50/50 p-3 dark:border-emerald-900 dark:bg-emerald-950/30">
            ...
            <p className="mt-2 text-base font-semibold leading-relaxed text-emerald-900 dark:text-emerald-100">{formula.examples[0]}</p>
          </li>
        ))}
      </ol>
    </div>
  ...
  return (
    <div className="flex h-full w-full flex-col justify-center gap-7 p-5 sm:p-8">
      ...
      <div className="mx-auto max-w-3xl text-center">
        <p className="text-sm font-bold text-rose-700 dark:text-rose-300">{content.controversialDefinition.eyebrow}</p>
        ...
        <p className="mt-4 text-base leading-relaxed text-muted-foreground">{guide.definition}</p>
        <p className="mt-5 rounded-lg border-l-4 border-rose-500 bg-rose-50 px-4 py-3 text-left text-base font-semibold leading-relaxed text-rose-950 dark:bg-rose-950/40 dark:text-rose-100">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {guide.formulas.map((formula, formulaIndex) => (
          <section key={formula.title} className="flex min-h-48 flex-col rounded-lg border border-rose-200 bg-background p-4 dark:border-rose-900">
            <span className="text-sm font-black text-rose-700 dark:text-rose-300">{content.controversialFormulas.judgmentLabel} {formulaIndex + 1}</span>
            <h4 className="mt-2 text-base font-bold leading-snug text-foreground">{formula.title}</h4>
            <p className="mt-3 text-base leading-relaxed text-muted-foreground">{formula.pattern}</p>
            <p className="mt-auto border-t border-rose-100 pt-3 text-base font-semibold leading-relaxed text-rose-950 dark:border-rose-900 dark:text-rose-100">
              {formula.examples[0]}
            </p>
          </section>
        ))}
    ...
    <div className="flex h-full w-full flex-col justify-center gap-5 p-5 sm:p-8">
      ...
      <div className="rounded-lg border bg-muted/20 p-5 text-center">
        <p
          ...
          aria-live="polite"
          aria-atomic="true"
          className="text-xl font-bold leading-relaxed text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-4"
        >
          {check.prompt}
        </p>
      </div>
        ...
        {choices.map((type) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex min-h-14 items-center justify-center gap-2 rounded-lg border px-3 text-base font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 motion-reduce:transition-none disabled:opacity-100",
                style.border,
                style.soft,
                style.text,
        ...
        >
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-base leading-relaxed text-muted-foreground">{check.explanation}</p>
            </div>
          </div>
  ...
  return (
    <div className="flex h-full w-full flex-col justify-center gap-7 p-5 sm:p-8">
        ...
        {content.inquirySteps.map((step, stepIndex) => {
          ...
          return (
            <li key={step.step} className={cn("relative rounded-lg border p-4", style.border, style.soft)}>
              ...
              <h4 className="mt-5 text-lg font-bold text-foreground">{step.title}</h4>
      ...
      <div className="flex items-center gap-3 rounded-lg bg-slate-900 px-4 py-4 text-white dark:bg-slate-100 dark:text-slate-950">
        ...
        <p className="text-base font-bold">{content.synthesis.finalPrompt}</p>
      </div>
