 ...
 * 단원 메타와 핵심 아이디어·성취기준·핵심 문장·핵심 질문·탐구 질문을 일관된 레이아웃으로 보여준다.
 */
export function DesignReferenceView({
  ...
  const coreSentenceNumber = hasAchievements ? 3 : 2;
  const essentialQuestionNumber = hasAchievements ? 4 : 3;
  const inquiryQuestionNumber = hasAchievements ? 5 : 4;
    ...
    <div className={className}>
      ...
      <div className="mt-3 space-y-3 text-sm">
              ...
              {achievements.map((achievement, index) => {
                ...
                return (
                  <article
                    ...
                    className="rounded-md border border-teal-200/70 bg-background/85 p-3 dark:border-teal-800/50"
                  >
                    <div className="flex flex-col gap-1.5 sm:flex-row sm:items-start sm:gap-3">
                      <span className="shrink-0 text-sm font-semibold text-teal-800 dark:text-teal-200">
                        {achievement.code}
                      </span>
                      <p className="text-sm leading-relaxed text-foreground">{achievement.content}</p>
                    </div>
                    {guide?.explanation && (
                      <div data-student-understanding-guide="achievement" className="mt-3 border-t border-teal-200/70 pt-3 text-xs dark:border-teal-800/50">
                        <p className="font-semibold text-foreground">{t("easyExplanation")}</p>
        ...
        {sentences.length > 0 && (
          ...
          >
            <div className="flex items-start gap-2.5">
              <span data-design-reference-number className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-sky-100 text-xs font-bold text-sky-800 dark:bg-sky-900/60 dark:text-sky-200" aria-hidden="true">{coreSentenceNumber}</span>
              <div className="min-w-0">
        ...
        {essential.length > 0 && (
          ...
          >
            <div className="flex items-start gap-2.5">
              <span data-design-reference-number className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-violet-100 text-xs font-bold text-violet-800 dark:bg-violet-900/60 dark:text-violet-200" aria-hidden="true">{essentialQuestionNumber}</span>
              <div className="min-w-0">
        ...
        {inquiry.length > 0 && (
          ...
          >
            <div className="flex items-start gap-2.5">
              <span data-design-reference-number className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-200" aria-hidden="true">{inquiryQuestionNumber}</span>
              <div className="min-w-0">
