    ...
    <div className="min-h-screen">
      ...
      {!isLoading && games.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {games.map((game, i) => (
            <GameCard key={game.id} game={game} index={i} onPlay={() => setSelectedGame(game)} />
          ))}
        </div>
      )}
          ...
          {selectedGame && (
            <>
              ...
              >
                ...
                <p className="text-white/90 text-sm mt-1 text-center px-6">
                  {selectedGame.description}
                </p>
                <div className="flex gap-3 mt-3">
                  <span className="bg-white/20 rounded-full px-3 py-0.5 text-xs font-medium">
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <div
          className="absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 w-56 h-56 rounded-full"
        ...
        <span className="absolute top-6 right-7 text-white/20 text-sm select-none">?</span>
        <span className="absolute bottom-6 left-7 text-white/20 text-sm select-none">?</span>
