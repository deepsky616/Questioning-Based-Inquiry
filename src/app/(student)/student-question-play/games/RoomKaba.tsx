...
export default function RoomKaba({
  ...
  async function submitQuestion() {
    ...
    if (!value || !isMyTurn || activeState.phase !== "question" || !prompt || pendingKind) return;
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5 text-foreground">
      <GameHeader game={game} subtitle={text.kabaSubtitle} onLeave={onLeave} leave={text.leave} />
          ...
          {prompt && (
            <section className="space-y-3 border-b border-border pb-5">
              <div className="bg-secondary p-4 rounded-lg">
                ...
                <p className="mt-1 font-semibold">{getLocalizedText(prompt.text, locale, prompt.key)}</p>
              </div>
              {isMyTurn && (
                <form className="flex flex-col gap-3 sm:flex-row" onSubmit={(event) => {
                  ...
                  />
                  <Button type="submit" disabled={!question.trim() || pendingKind !== null}>
        ...
        {state.attempts.length === 0 ? (
          ...
          <ol className="space-y-2">
            {state.attempts.map((attempt) => (
              <li key={`${attempt.roundId}:${attempt.playerId}`} className="border border-border bg-card p-3 rounded-lg">
                ...
                <p className="mt-1 text-sm">{attempt.question}</p>
              </li>
            ))}
          </ol>
        )}
