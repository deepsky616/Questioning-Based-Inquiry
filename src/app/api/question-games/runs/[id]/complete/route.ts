...
import { completeQuestionGameRun } from "@/lib/question-game-run-service";
...
export async function POST(req: Request, { params }: Params) {
  ...
  const limited = checkRateLimit(`question-game-run-complete:${actorId}`, 10);
  if (limited) return limited;

  try {
    const { id } = await params;
    const result = await completeQuestionGameRun(
      actorId,
      id,
