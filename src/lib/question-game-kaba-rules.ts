...
function koreanPredicate(
  source: string,
  informalStems: readonly string[],
  ggalkaStems: readonly string[] = informalStems,
): RegExp {
  const informal = informalStems.map(escapeRegExp).join("|");
  const ggalka = ggalkaStems.map(escapeRegExp).join("|");
  return new RegExp(
    `(?:${source}|(?:${informal})(?:니|냐)|(?:${ggalka})(?:까|까요))\\s*[?？]?$`,
    "u",
  );
}

const KO_RULES: readonly KabaMeaningRule[] = [
  [[koreanTerm("고양이")], [koreanPredicate(String.raw`자나요|잡니까|자는지요|자고\s*있나요`, ["자"], ["잘"])]],
  [[koreanTerm("개미")], [koreanPredicate(String.raw`걷나요|걷습니까|걷는지요|걷고\s*있나요`, ["걷"], ["걸을"])]],
  [[koreanTerm("토끼")], [koreanPredicate(String.raw`뛰나요|뜁니까|뛰는지요|뛰고\s*있나요`, ["뛰"], ["뛸"])]],
  [[koreanTerm("꽃")], [koreanPredicate(String.raw`예쁜가요|예쁘나요|예쁩니까|예쁜지요|아름다운가요`, ["예쁘"], ["예쁠"])]],
  [[koreanTerm("사과")], [koreanPredicate(String.raw`빨간가요|빨갛나요|빨갛습니까|빨갑니까|빨간지요|붉은가요`, ["빨갛"], ["빨갈"])]],
  [[koreanTerm("하늘")], [koreanPredicate(String.raw`파란가요|파랗나요|파랗습니까|파랍니까|파란지요|푸른가요`, ["파랗"], ["파랄"])]],
  [[koreanTerm("비")], [koreanPredicate(String.raw`오나요|옵니까|오는지요|내리나요|내립니까`, ["오", "내리"], ["올", "내릴"])]],
  [[koreanTerm("새")], [koreanPredicate(String.raw`날아가나요|날아갑니까|날아가는지요|비행하나요`, ["날아가", "비행하"], ["날아갈", "비행할"])]],
  [[koreanTerm("강아지|개")], [koreanPredicate(String.raw`짖나요|짖습니까|짖는지요`, ["짖"], ["짖을"])]],
  [[koreanTerm("물고기")], [koreanPredicate(String.raw`헤엄치나요|헤엄칩니까|헤엄치는지요|수영하나요`, ["헤엄치", "수영하"], ["헤엄칠", "수영할"])]],
  [[koreanTerm("아이")], [koreanPredicate(String.raw`웃나요|웃습니까|웃는지요|미소를?\s*짓나요`, ["웃", "미소를 짓"], ["웃을", "미소를 지을"])]],
  [[koreanTerm("나무")], [koreanPredicate(String.raw`흔들리나요|흔들립니까|흔들리는지요`, ["흔들리"], ["흔들릴"])]],
  [[koreanTerm("별")], [koreanPredicate(String.raw`빛나나요|빛납니까|빛나는지요|반짝이나요|반짝입니까`, ["빛나", "반짝이"], ["빛날", "반짝일"])]],
  [[koreanTerm("바람")], [koreanPredicate(String.raw`부나요|붑니까|부는지요|불고\s*있나요`, ["부"], ["불"])]],
  [[koreanTerm("눈")], [koreanPredicate(String.raw`내리나요|내립니까|내리는지요|오나요|옵니까`, ["내리", "오"], ["내릴", "올"])]],
  [[koreanTerm("나비")], [koreanTerm("날개")], [koreanPredicate(String.raw`펴나요|펴는지요|펼치나요|펼칩니까|펼치는지요`, ["펴", "펼치"], ["펼", "펼칠"])]],
  [[koreanTerm("달")], [koreanPredicate(String.raw`밝은가요|밝나요|밝습니까|밝은지요`, ["밝"], ["밝을"])]],
  [[koreanTerm("파도")], [koreanPredicate(String.raw`치나요|칩니까|치는지요|밀려오나요|부서지나요`, ["치", "밀려오", "부서지"], ["칠", "밀려올", "부서질"])]],
  [[koreanTerm("벌")], [koreanTerm("꿀")], [koreanPredicate(String.raw`모으나요|모읍니까|모으는지요|모아\s*오나요`, ["모으"], ["모을"])]],
  [[koreanTerm("원숭이")], [koreanTerm("나무")], [koreanPredicate(String.raw`오르나요|오릅니까|오르는지요|올라가나요|올라갑니까`, ["오르", "올라가"], ["오를", "올라갈"])]],
  [[koreanTerm("햇빛|햇살")], [koreanPredicate(String.raw`따뜻한가요|따뜻하나요|따뜻합니까|따뜻한지요`, ["따뜻하"], ["따뜻할"])]],
  [[koreanTerm("구름")], [koreanPredicate(String.raw`하얀가요|하얗나요|하얗습니까|하얍니까|하얀지요|흰가요`, ["하얗"], ["하얄"])]],
  [[koreanTerm("고래")], [koreanTerm("바다")], [koreanPredicate(String.raw`사나요|삽니까|사는지요|살고\s*있나요`, ["사"], ["살"])]],
  [[koreanTerm("개구리")], [koreanPredicate(String.raw`우나요|웁니까|우는지요|울까요|울고\s*있나요|개굴(?:거리나요|거립니까|대나요)`, ["우", "개굴거리", "개굴대"], ["울", "개굴거릴", "개굴댈"])]],
