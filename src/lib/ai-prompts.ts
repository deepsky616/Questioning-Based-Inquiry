...
- 질문 유형(폐쇄/개방, 사실/개념/논쟁) 분포가 탐구에 적절히 균형 잡혔는지 진단하고, 한쪽에 치우쳤으면 부족한 유형을 끌어낼 방법을 제안하세요.
...
export function buildStudentSessionPrompt(a: StudentSessionActivity): string {
  ...
  const priorText = a.prior && a.prior.totalQuestions > 0
    ? `이전까지 누적 질문 ${a.prior.totalQuestions}개 중 개방형 ${a.prior.open}개·개념형 ${a.prior.conceptual}개·논쟁형 ${a.prior.controversial}개`
...
- 이전 활동과 비교해 이번에 나아진 점(개방형·개념형·논쟁형이 늘었는지 등)을 찾아 칭찬하세요. 비교할 게 없으면 앞으로의 성장 방향을 알려주세요.
