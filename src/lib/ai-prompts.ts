...
// 닫힌 질문/열린 질문 × 인지적 수준의 2차원 조합별 답변 지침
const ANSWER_GUIDE: Record<string, Record<string, string>> = {
  factual: {
    closed:
      "사실적·닫힌 질문: 핵심 사실을 간결하게 확인해 주세요. 마지막에 관련 원리나 비슷한 예시를 하나 덧붙여 이해를 자연스럽게 확장하세요.",
    open:
      "사실적·열린 질문: 여러 측면의 사실을 함께 제시하고, 어떤 부분이 더 궁금한지 생각해보도록 유도하는 문장으로 마무리하세요.",
  },
  conceptual: {
    closed:
      "개념적·닫힌 질문: 이유나 원리를 단계적으로 설명하세요. '그렇다면 다른 상황에서는 어떨까요?'처럼 사고를 확장하는 질문으로 마무리하세요.",
    open:
      "개념적·열린 질문: 여러 가지 설명 가능성을 열어두고, 개념 사이의 관계를 스스로 생각해보도록 격려하세요.",
  },
  controversial: {
    closed:
      "논쟁적·닫힌 질문: 판단의 기준을 명확히 제시하되, 기준이 달라지면 답도 달라질 수 있음을 보여주세요. 학생 스스로 자신의 기준을 세워보도록 돕는 마무리를 포함하세요.",
    open:
      "논쟁적·열린 질문: 다양한 관점이 모두 가능함을 인정하고, 좋은 판단을 위해 어떤 기준을 고려해야 하는지 생각해보도록 유도하세요. 정답을 단정하지 마세요.",
  },
};
...
- 닫힌 질문 / 열린 질문: ${closedCount} / ${openCount}
- 사실적 / 개념적 / 논쟁적: ${factualCount} / ${conceptualCount} / ${controversialCount}

[질문 유형 기준]
- 닫힌 질문: 정답이 하나인 확인형 질문
- 열린 질문: 다양한 답이 나올 수 있는 탐구형 질문
- 사실적: 사실·정보 확인
- 개념적: 추론·분석·비교를 통해 개념과 원리의 관계를 탐색
- 논쟁적: 판단·의견·가치 기준을 세워 여러 관점을 비교

[표기 안내]
- 각 질문 앞 [ ] 안: 질문 종류(학생/배포)·닫힌·열린·인지수준·좋아요 수(❤️)
...
- 질문 유형(닫힌/열린, 사실/개념/논쟁) 분포가 탐구에 적절히 균형 잡혔는지 진단하고, 한쪽에 치우쳤으면 부족한 유형을 끌어낼 방법을 제안하세요.
- 이번 세션에서 가장 좋은 질문 1~2개를 골라 왜 좋은지 설명하세요(수업에서 모범 사례로 쓸 수 있게).
- 이 세션의 질문들을 발전시킨, 다음 수업에서 바로 쓸 수 있는 발문 2~3개를 제안하세요.
...
아래 JSON 형식으로만 응답하세요:
{
  ...
  "balanceInsights": "질문 유형 분포가 균형 잡혔는지 진단 + 부족한 유형을 끌어낼 방법을 2~3문장",
  "bestQuestion": "가장 좋은 질문 1~2개를 따옴표로 인용하고 왜 좋은지 한 문장씩. 형식 예: \\"질문내용\\" — 이유",
  "nextQuestions": "다음 수업에서 쓸 발문 2~3개를 줄바꿈으로 구분(1) 2) 3) 번호 포함)"
}`;
}
...
export interface StudentSessionActivity {
  ...
  myComments: string[];
  likesGiven: number;
  // 지난 세션 대비 성장 비교용(이전까지 누적 질문 분포). 없으면 첫 활동으로 간주.
  prior?: { totalQuestions: number; open: number; conceptual: number; controversial: number };
}
...
export function buildStudentSessionPrompt(a: StudentSessionActivity): string {
  ...
  const cList = a.myComments.length > 0
    ...
    : "(작성한 댓글 없음)";
  const priorText = a.prior && a.prior.totalQuestions > 0
    ? `이전까지 누적 질문 ${a.prior.totalQuestions}개 중 열린 질문 ${a.prior.open}개·개념형 ${a.prior.conceptual}개·논쟁형 ${a.prior.controversial}개`
    : "(이전 활동 기록 없음 — 이번이 첫 활동에 가까움)";
...
[학생] ${a.studentName}
[내가 만든 질문] (앞 [ ] 안: 닫힌/열린·인지수준·받은 좋아요(❤️)·받은 댓글(💬))
${qList}
...
- 이전 활동과 비교해 이번에 나아진 점(열린 질문·개념형·논쟁형이 늘었는지 등)을 찾아 칭찬하세요. 비교할 게 없으면 앞으로의 성장 방향을 알려주세요.
- 내가 쓴 질문 중 짧거나 아쉬운 것 하나를 골라, 주제와 연결해 더 좋은 질문으로 바꾼 예시를 보여주세요.
...
반드시 아래 JSON 형식으로만 응답하세요 (다른 말 없이):
{
  ...
  "growthInsights": "이전 활동 대비 나아진 점을 구체적으로 칭찬 1~2문장. 비교 기록이 없으면 앞으로의 성장 방향 제시",
  "rewriteExample": "내 질문 중 하나를 더 좋은 질문으로 바꾼 예시. 형식: \\"원래 질문\\" → \\"더 좋은 질문\\" (왜 더 좋은지 한 문장)"
}`;
}
