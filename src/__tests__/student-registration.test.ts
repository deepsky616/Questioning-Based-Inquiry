...
describe("partitionStudents", () => {
  ...
  it("이미 존재하는 학생은 skipped로 분류된다", () => {
    const students: StudentInput[] = [
      { studentNumber: "1", name: "홍길동" },
      { studentNumber: "2", name: "김철수" },
    ];
    const existingEmails = new Set<string>(["s_서울초_3_2_1@student.internal"]);

    const result: BulkPartition = partitionStudents(students, classInfo, existingEmails);
