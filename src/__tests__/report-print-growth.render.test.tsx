...
it("분석이 없는 수업도 직접 쓴 성장 기록을 모두 출력하며 빈 기록과 빈 항목은 생략한다", () => {
  ...
  records[104] = { ...records[104], revisedContent: records[104].originalContent, changeNote: " \n ", reflection: "마지막으로 배운 점" };
  const item = {
  ...
  for (const text of ["처음 질문 0", "고친 질문 0", "돌아보기 0", "조건을 같게 맞추어야 해요.", "마지막으로 배운 점"]) expect(screen.getByText(text, { exact: true })).toBeVisible();
  expect(screen.queryByText("자동 보관만 한 질문")).not.toBeInTheDocument();
  expect(screen.getAllByText("탐구하며 알게 된 점", { exact: true })).toHaveLength(2);
  expect(screen.getAllByText("처음 질문 104", { exact: true })).toHaveLength(1);
  expect(screen.getByText("나의 질문", { exact: true })).toBeVisible();
  expect(screen.queryByText("미작성", { exact: true })).not.toBeInTheDocument();
  expect(screen.queryByText("질문을 수정하지 않음", { exact: true })).not.toBeInTheDocument();
