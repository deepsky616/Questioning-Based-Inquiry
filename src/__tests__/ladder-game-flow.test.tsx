...
function renderGame(mode: "solo" | "ai" = "solo") {
...
describe("지역 질문 사다리 세 라운드", () => {
  ...
  it("첫째와 둘째 질문 뒤 새 사다리를 거치고 셋째 질문 뒤에만 셋을 완료로 보여 준다", async () => {
    ...
    drawLadder();
    expect(Math.random).toHaveBeenCalledTimes(30);
    chooseStart(1);
    ...
    expect(screen.queryByText("질문 사다리 완성")).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "다음 라운드" }));
    expect(Math.random).toHaveBeenCalledTimes(90);
  ...
  it("분류 성공 도움말을 보고 공통 작성기에서 질문을 확정한다", async () => {
    ...
    const input = screen.getByRole("textbox", { name: "주제 A 주제 질문" });
    fireEvent.change(input, {
      target: { value: "주제 A는 왜 다른 주제와 이어질까요?" },
    });
    fireEvent.click(screen.getByRole("button", { name: "질문 확인" }));
    ...
    expect(await screen.findByText("첫째 라운드 질문")).toBeInTheDocument();
    expect(screen.getByText("주제 A는 왜 다른 주제와 이어질까요?")).toBeInTheDocument();
  });
});

describe("인공지능과 함께하는 질문 사다리", () => {
  it("학생과 인공지능 배정을 같은 사다리에 두고 서로의 실제 도착 주제를 쓴다", async () => {
    aiMocks.ask.mockResolvedValue({ text: "주제 A는 무엇과 이어질까요?\n둘째 줄" });
    vi.stubGlobal("fetch", vi.fn(async () => jsonResponse({}, 500)));
    renderGame("ai");
    drawLadder();
    ...
    expect(studentRow).toHaveTextContent("주제 B");
    expect(aiRow).toHaveTextContent("1");
    expect(aiRow).toHaveTextContent("주제 A");
    expect(
      screen.getByRole("textbox", { name: "주제 B 주제 질문" }),
    ).toBeInTheDocument();
    expect(await screen.findByText("주제 A는 무엇과 이어질까요?")).toBeInTheDocument();
    expect(
