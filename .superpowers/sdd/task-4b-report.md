...
- 공용 `RoomResult`를 유지해 방장의 대기실 다시 시작을 보존했다. 버전 2 결과 공유와 점수 지급 연결은 작업 8 범위로 남겼다.
...
- 실패 공개마다 명령 식별값, 관찰한 방 버전, 마지막 요청 버전, 요청 및 타이머 상태를 하나의 진행 참조에 보존한다.
- 복원 요청이 실패하거나 예외로 끝나면 더 높은 같은 공개 버전을 기다린다. 버전 상승과 실패 응답의 순서가 바뀌어도 같은 명령 식별값으로 복원을 재개한다.
- 정상 `retryAfterMs` 타이머가 기다리는 동안 버전이 올라가도 요청을 더 보내거나 타이머를 다시 시작하지 않는다.
- 공개 또는 실행 수명이 바뀌면 옛 타이머와 진행 참조를 정리한다. 지연된 옛 요청이 나중에 성공해도 새 수명에 재시도를 만들지 않는다.
- 결과 화면 시험은 `readMemoryState`를 통과하는 실제 `done/completed` 상태를 쓰고, 점수 공유 성공을 전제로 하지 않은 채 방장 다시 시작만 확인한다.
- 버전 2 주사위 훅 시험은 엄격한 `rolling` 상태와 서버 형식의 실행 및 라운드 식별값을 쓴다. 요청 본문 전체를 비교해 옛 `roll`과 `rollRoundId`가 섞이지 않음을 고정했다.
- 마지막 지역 성공으로 얻은 한 짝이 `validQuestions: 1`과 `completed: true`로 적립 요청에 포함되는지 직접 확인했다.

### 실패 시험 확인

```text
npx vitest run src/__tests__/room-memory-actions.test.tsx -t '실패 뒤 같은 공개|높은 버전이 요청 실패|네트워크 예외 뒤|정상 재시도 대기 중'
PASS 1, FAIL 3: 실패 및 예외 뒤 높은 같은 공개 버전에서 복원 요청이 재개되지 않음
```
...
npx eslint 'src/app/(student)/student-question-play/games/RoomMemory.tsx' src/__tests__/room-memory-actions.test.tsx src/__tests__/use-room.test.tsx
통과
...
- `12008a9` `fix: 실패 공개 복원 재개 보강`

### 남은 우려

- 인공지능 방식의 마지막 실패 뒤 새 인공지능 차례가 예약되지 않는지는 별도 장기 타이머 시험으로 직접 고정하지 않았다. 이번 재검토는 복원 순서 경합과 요청 본문 경계로 범위를 제한했다.
- 공용 `RoomResult`의 버전 2 결과 공유와 메모리 점수 지급 근거 연결은 계획의 작업 8 범위다. 현재 시험은 방장 다시 시작만 보존한다.
- 복원 대기 상한 2500밀리초는 서버 판정기와 화면에 각각 정의되어 있다.
- 저장 구조와 데이터베이스 파일은 바꾸지 않았고, 기존 사용자 커밋 `defab6a`는 수정하거나 되돌리지 않았다.
