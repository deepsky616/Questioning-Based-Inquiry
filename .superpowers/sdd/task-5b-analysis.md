...
### 방 서버의 활동은 질문과 추측의 전체 횟수다

작업 5가의 최종 승인 규약은 질문과 추측을 같은 활동 단위로 센다.

- `mystery-ask`는 질문 기록과 질문자 점수를 늘리고 활동 한 번을 사용한다.
- `unknown` 답도 활동 한 번을 사용한다.
- 질문과 틀린 추측은 새 `roundId`, 다음 라운드와 다음 차례를 만든다.
- 맞은 추측 또는 스무 번째 질문이나 틀린 추측이 방을 끝낸다.

따라서 방 화면의 사용 횟수는 전체 `history.length`다.

```ts
const activityCount = state.history.length;
const remaining = state.maxRounds - activityCount;
```

`state.round`는 진행 중이면 다음 활동 번호이고, 완료 뒤에는 마지막 활동 번호다. `scores`는 참가자별 질문 수다. 최고 질문 수로 승자를 다시 계산하면 안 되고 승자는 오직 `winnerId`로 정한다.
...
질문 성공은 새 `roundId`와 다음 차례를 만든다. `await onAction(...)`이 `ok`일 때 질문 입력을 지우고 서버가 돌려준 새 라운드와 차례를 따른다. 반대로 요청 전에 지우면 거절과 충돌에서 학생 글을 잃는다.

### 연결 오류 뒤 명령 식별값 재사용

`useRoom`은 호출마다 새 명령 식별값을 만든다. 연결이 끊긴 뒤 같은 질문을 새 식별값으로 다시 보내면 첫 요청이 이미 저장된 경우 질문이 두 번 기록될 수 있다.
...
- 진행: 활동 라운드와 남은 활동, 질문 수, 현재 차례와 전체 기록을 보인다.
- 질문 답: `yes`, `no`, `unknown`을 현재 언어의 `text.yes`, `text.no`, `text.notSure`로 바꾼다.
- 입력: 질문과 추측을 서로 다른 값과 단추로 둔다. 차례 밖, 요청 중, 식별값 없음에서는 둘 다 잠근다.
- 결과: `winnerId`가 있으면 정답 성공, 없고 `completed`면 스무 활동 종료, `insufficient-players`면 참가자 부족으로 구분한다.
...
친구 방과 지역 화면은 모두 질문 또는 추측 한 번을 한 활동으로 센다. 두 화면 모두 `남은 활동`과 현재 라운드를 같은 뜻으로 표시한다.
...
- 방장 시작과 비방장 거절
- 질문은 점수를 늘리고 새 라운드와 다음 차례로 이동
- 틀린 추측의 새 라운드와 차례 이동
- 맞은 추측 조기 종료와 스무 번째 질문 또는 오답 종료
- 같은 명령 재생과 재접속 상태 유지
- 참가자 이탈과 부족 인원 종료
- 저장 상태와 공개 상태 판독 분리
...
5. 두 참가자 그리기에 같은 기록, 판정, 라운드, 남은 활동과 현재 차례가 보인다.
...
12. 질문 성공 뒤 새 라운드와 다음 차례를 따르며 질문 입력이 비워진다.
13. 연결 오류 뒤 같은 입력 재시도는 같은 명령 식별값을 쓴다. 공개 `recentCommandIds`로 반영을 확인하면 입력을 정리한다.
14. 스무 번째 전에는 질문과 추측을 모두 할 수 있고 스무 번째 활동 뒤에는 두 입력이 모두 닫힌다.

#### 결과와 다시 시작

15. 맞은 추측 완료는 `winnerId`, 과거 기록 이름, 공개 정답과 질문 수를 보인다.
16. 스무 번째 질문 또는 오답 완료는 승자를 만들지 않고 스무 활동 종료와 공개 정답을 보인다.
...
1. 방 화면에서 추측 기록만 세면 질문 활동이 빠져 서버 라운드와 화면의 남은 활동이 어긋난다.
